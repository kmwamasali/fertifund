...
class Header extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if(nextProps.deletedCampaign.error && nextProps.deletedCampaign.error.message) {//delete failure
      alert(nextProps.deletedCampaign.error.message || 'Could not delete. Please try again.');
    } else if(nextProps.deletedCampaign.post && !nextProps.deletedCampaign.error) {//delete success
  ...
  renderSignInLinks(authenticatedUser) {
	...
	renderLinks() {
		...
		if(type === 'campaigns_index') {
       return (
        <div className="container">
          <ul className="nav  nav-pills navbar-right">
      			<li style={{paddingRight: '10px'}} role="presentation">      
      				<Link style={{color:'#337ab7',  fontSize: '17px'}} to="/campaigns/new">
      				New Campaign
    					</Link>
            </li>
    			</ul>
  			...
  			return (
  			 <div className="container">
    			...
    			<div className="navbar-form navbar-right" style={{paddingRight: '50px'}}>
      			<button className="btn btn-warning pull-xs-right"  onClick={()=> {this.props.onDeleteClick()}}>Delete Campaign</button>
      		</div>
           {this.renderSignInLinks(authenticatedUser)}
    	   </div>
  		);
